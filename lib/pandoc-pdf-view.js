...
export default class PandocPdfView {
  ...
  constructor(editor) {
    ...
    this.element.innerHTML = `
      ...
      <span class="inline-block-tight btn icon icon-question"
        data-id="show-help"
        title="Show Pandoc User's Guide"></span>
    ...
    // handle clicks on log (links & copy)
    this.logText.addEventListener('click', (event) => this.handleClick(event))

    // prevent scrolling on dialog to also scroll editor beneath
    this.dialog.addEventListener('wheel', (e) => event.stopPropagation())

    // destroy this view if editor is destroyed
    this.subscriptions.add(
  ...
  async handleClick(event) {
    ...
    if (event.target.getAttribute('data-ref') == 'source') {
      ...
      const lineNo = event.target.getAttribute('data-line')
      const columnNo = event.target.getAttribute('data-column')
      // move cursor to that position
      this.editor.setCursorBufferPosition([lineNo - 1, columnNo - 1])
      // dismiss dialog
      this.showLog(false)
      return
    }
    ...
    if (event.target.getAttribute('data-ref') == 'tex') {
      ...
      const lineNo = parseInt(event.target.getAttribute('data-line'))
    ...
    // copy output line to clipboard
    const line = event.target.closest('.pandoc-pdf-log p')
    if (line) {
      atom.clipboard.write(line.textContent)
      // blink background as feedback
      line.classList.add('selected')
      window.setTimeout(() => line.classList.remove('selected'), 100)
    }
  }


  // show Pandoc/PDF reference in pane
  showHelp() {
    const url = 'https://pandoc.org/MANUAL.pdf'
