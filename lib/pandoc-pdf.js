...
export default {
    ...
    'pandocInputFormats': {
      ...
      'properties': {
        'fileExtensions': {
          ...
          an input format should be configured. Extensions can include \`.\`s.
  ...
  activate(state) {
    ...
    // register command that adds the Pandoc/PDF toolbar and processes
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
        'pandoc-pdf:process': () => this.addAndProcess()
      })
    )
    // register command that removes the Pandoc/PDF toolbar
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
        'pandoc-pdf:remove': () => this.remove()
      })
    )
    ...
    // hook into core:cancel (ESC) to close the log view
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
  ...
  // add Pandoc/PDF toolbar & start processing
  async addAndProcess() {
